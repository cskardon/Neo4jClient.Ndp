namespace Neo4jNdpClient
{
    ...
    public static partial class Packers
    {
        ...
        {
            private static readonly Type[] AllowedTypes = {typeof (List<>), typeof (List), typeof (IList)};
            ...
            private static int GetNumberOfElements(byte[] content)
            {
                ...
                if (content[0] == 0xD5)
                    return int.Parse(BitConverter.ToString(content.Skip(1).Take(2).ToArray()).Replace("-", ""), NumberStyles.HexNumber);

                if (content[0] == 0xD6)
                    return int.Parse(BitConverter.ToString(content.Skip(1).Take(4).ToArray()).Replace("-", ""), NumberStyles.HexNumber);
